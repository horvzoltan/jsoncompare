...
document.getElementById("validate").addEventListener("click", function (ev) {
  if (!files || !secondFile || !isValid) {
    errorElement.textContent = 'You need to select files in both files selectors.'
    return
  }
...
const loadFile = async (ev, container) => {
  ...
  await Array.from(ev.currentTarget.files).forEach(file => currentFiles.push(file));
  ...
  for (const value of currentFiles) {
    const index = currentFiles.indexOf(value);
      if (index > 0) {
        if (!await validateStructure(filesTestArray[0], await readFileAsText(value))) {
          console.log('123')
          return;
        }
      }
